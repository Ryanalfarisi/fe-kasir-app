...
const saveUser = async () => {
  try {
    if (isEditing.value) {
      await authAxios.put(`/users/${form.value.id}`, form.value)
    } else {
      await authAxios.post('/users', form.value)
    }
...
const deleteUser = async (id) => {
  if (!confirm('Hapus karyawan ini? Data yang dihapus tidak bisa dikembalikan.')) return
  try {
...
<template>
  <DashboardLayout>
    ...
    <!-- Error State -->
    <div v-if="errorMessage" class="bg-red-50 text-red-700 p-4 rounded-xl border border-red-200 mb-6 flex items-center gap-3">
        ...
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="users.length === 0">
              <td colspan="4" class="p-8 text-center text-gray-500">Tidak ada karyawan ditemukan</td>
            </tr>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          <h3 class="font-bold text-gray-800">{{ isEditing ? 'Edit Karyawan' : 'Tambah Karyawan' }}</h3>
        ...
        <form @submit.prevent="saveUser" class="p-6 space-y-4">
          <div>
            ...
            <input v-model="form.name" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Nama Karyawan" />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Username</label>
            <input v-model="form.username" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Username untuk login" />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
              Password
              <span v-if="isEditing" class="text-xs text-gray-400 font-normal">(Kosongkan jika tidak ingin ubah)</span>
            </label>
            <input v-model="form.password" :required="!isEditing" type="password" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="*****" />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Role / Jabatan</label>
            <select v-model="form.role" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white">
